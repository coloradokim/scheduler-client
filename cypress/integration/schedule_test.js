const DEPLOYED_URL = 'scheduler-client.firebaseapp.com';
const LOCAL_URL = 'localhost:8080';

describe('Check landing page', function() {
  it('visit the landing page and check the title', function() {
    cy.visit(LOCAL_URL);
    cy.title().should('include', 'Scheduler');
  });
});

describe('Checks Form', function() {
  it('checks that all elements of the form work', function() {
    cy.visit(LOCAL_URL);
    cy.get('form').submit();
  });
});
